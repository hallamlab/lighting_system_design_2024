...
def parseInfo(file_data, file_data_df):
    ...
    for file in file_data:
        ...
        # Get wavelength from third line of csv file using csv library
        data_sheet = []
    ...
    file_data_df['datetime'] = pd.to_datetime(file_data_df[2], format = "%y-%m-%d %H-%M")
...
def sorted384_2x2(sorted_data_list):
    # Build sorted output file
    OD_data = [['measurement_ID', plate_id_header, 'timepoint', 'time', 'datetime', 'wavelength', 'well', 'OD1', 'OD2', 'OD3', 'OD4']]
    ...
    for file in sorted_data_list[1:]:
        with open(data_path + file[0], encoding='unicode_escape') as csv_file:
            ...
            for row in csv_reader:
                csv_list.append(row)
            measured_wells = int((len(csv_list)-2)/5)
            for i in range(0,measured_wells):
                well = csv_list[(i*5)+1][0]
                v1 = csv_list[(i*5)+3][0]
                v2 = csv_list[(i*5)+3][1]
                v3 = csv_list[(i*5)+4][0]
                v4 = csv_list[(i*5)+4][1]
                data_row = [file[4],
                            ...
                            v3,
                            v4]
                full_data_row = data_row + row[:-1]
                OD_data.append(full_data_row)
...
def __main__(data_path = data_path):
    ...
    OD_data = sorted384_2x2(sorted_data_list)
